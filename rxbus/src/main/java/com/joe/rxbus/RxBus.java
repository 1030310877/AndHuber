...
public class RxBus {
    ...
    private List<WeakReference<Object>> registerMasters;
    ...
    private boolean checkRegister(Object master) {
        for (WeakReference<Object> item : registerMasters) {
    ...
    private void removeMasterFromList(Object master) {
        int index;
        for (index = 0; index < registerMasters.size(); index++) {
            if (registerMasters.get(index) != null && registerMasters.get(index).get().equals(master)) {
                break;
            }
        }
        if (index >= 0 && index < registerMasters.size()) {
            registerMasters.remove(index);
        }
    }
